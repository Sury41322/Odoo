...
class PaymentTransaction(models.Model):
    ...
    def action_paytrial_set_done(self):
        """ Set the state of the paytrial transaction to 'done'.
        ...
        """
        self.ensure_one()
        if self.provider_code != 'paytrial':
            return

        notification_data = {'reference': self.reference, 'simulated_state': 'done'}
        self._handle_notification_data('paytrial', notification_data)

    def action_paytrial_set_canceled(self):
        """ Set the state of the paytrial transaction to 'cancel'.
    ...
    def action_paytrial_set_error(self):
        """ Set the state of the paytrial transaction to 'error'.
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError("paytrial: " + _("The transaction is not linked to a token."))

        simulated_state = self.token_id.paytrial_simulated_state
    ...
    def _send_refund_request(self, **kwargs):
        ...
        refund_tx._handle_notification_data('paytrial', notification_data)
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        tx._handle_notification_data('paytrial', notification_data)
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'paytrial' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'paytrial')])
        if not tx:
            raise ValidationError(
                "paytrial: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_notification_data(self, notification_data):
        ...
        self.provider_reference = f'paytrial-{self.reference}'
        ...
        if self.tokenize:
            ...
            self._paytrial_tokenize_from_notification_data(notification_data)
        ...
        if state == 'pending':
            ...
            self._set_error(_("You selected the following paytrial payment status: %s", state))

    def _paytrial_tokenize_from_notification_data(self, notification_data):
        ...
        token = self.env['payment.token'].create({
            ...
            'paytrial_simulated_state': state,
        })
        self.write({
            'token_id': token,
            'tokenize': False,
