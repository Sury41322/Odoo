...
import paymentpaytrialMixin from '@payment_paytrial/js/payment_paytrial_mixin';
    ...
    async _initiateExpressPayment(ev) {
        ...
        if (shippingInformationRequired){
            const shippingInfo = document.querySelector(
                `#o_payment_paytrial_shipping_info_${providerId}`
            );
            expressDeliveryAddress = {
                'name': shippingInfo.querySelector('#o_payment_paytrial_shipping_name').value,
                'email': shippingInfo.querySelector('#o_payment_paytrial_shipping_email').value,
                'street': shippingInfo.querySelector('#o_payment_paytrial_shipping_address').value,
                'street2': shippingInfo.querySelector('#o_payment_paytrial_shipping_address2').value,
                'zip': shippingInfo.querySelector('#o_payment_paytrial_shipping_zip').value,
                'city': shippingInfo.querySelector('#o_payment_paytrial_shipping_city').value,
                'country': shippingInfo.querySelector('#o_payment_paytrial_shipping_country').value,
            };
        ...
        paymentpaytrialMixin.processpaytrialPayment(processingValues);
    },
});
